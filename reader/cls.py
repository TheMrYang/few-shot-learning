...
train_classes = [0,10,12,13,14,15,16,19,2]
eval_classes = [4,5,6,8,9]
#eval_classes = [0,10,12,13,14,15,16,19,2]
...
class FewshotProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "all.txt")), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "all.txt")), "dev")
    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "all.txt")), "test")
    def get_labels(self):
        """See base class."""
        return [0,1]
        """
    ...
    def generate_batch_data(self,
                            ...
                            return_input_mask=True,
                            return_max_len=False,
                            return_num_token=False,few_shot=False,k=0,n=0,batch_label=[]):
        return prepare_batch_data(
            batch_data,
            total_token_num,
            voc_size=-1,
            ...
            return_input_mask=True,
            return_max_len=False,
            return_num_token=False,k=k,n=n,batch_label=batch_label)
    def _create_examples(self, lines, set_type):
        """Creates examples for the training and dev sets."""
        examples = []
        for (i, line) in enumerate(lines):
            guid = "%s-%s" % (set_type, i)
            content = tokenization.convert_to_unicode(line[0])
            label = tokenization.convert_to_unicode(line[1])
            #select classes for train evl test
            classes = []
            if set_type == "train":
            ...
            if int(label) in classes:
                ...
                examples.append(
                    InputExample(
                    guid=guid, text_a=content, text_b="", label=int(label)))
        return examples
    ...
    def data_generator(self,
        ...
        def instance_reader():
            ...
            for example in examples:
                if example.label not in class_examples:
                    class_examples[example.label] = []
            ...
            if phase == 'train':
                ...
                iter_sum = test_iter
            for iter in range(iter_sum):
                self.current_train_example = self.current_train_example + 1
                self.current_train_epoch = self.current_train_epoch + 1
                target_classes = []
                if phase == 'train':
                     target_classes = random.sample(train_classes, k)
                else:
                     target_classes =  random.sample(eval_classes, k)
                support_set = []
                query_set = []
                #if phase != 'train':
                    #print(target_classes)
                for i, class_name in enumerate(target_classes):
                    random_list = random.sample(class_examples[class_name], n + q) 
                    support_set = support_set + random_list[0:n]
                    query_set = query_set + random_list[n:]
                
                #one step
                task = []
                label_list = []
                for i, query in enumerate(query_set):
                    label_list = label_list  + [i / q]
                    for support in support_set:
                        #print(support.text_a)
                        sample = InputExample(guid="", text_a=query.text_a, text_b=support.text_a, label=int(i))
                        feature = self.convert_example(0,sample, None, self.max_seq_len, self.tokenizer)
                        instance = self.generate_instance(feature)
                        task.append(instance)
                #print(task)
                #print(len(label_list))
                yield task,label_list


            """
            target_classes = random.sample(self.get_labels(), n)
            for i, class_name in enumerate(target_classes):
                random.sample(class_examples[class_name], k + q)
        ...
        def batch_reader(reader, batch_size, in_tokens):
            ...
            batch_label = []
            count = 0
            for instance,label_list in reader():
                #print(instance)
                if count < batch_size:
                    batch = batch + instance
                    batch_label = batch_label + label_list
                    count = count + 1
                else:
                    #print(batch)
        ...
        def wrapper():
            all_dev_batches = []
            total_token_num = 0
            for batch_data,batch_label in batch_reader(
                    instance_reader, batch_size, self.in_tokens):
                batch_data = self.generate_batch_data(
                    batch_data,
                    total_token_num,
                    ...
                    return_input_mask=True,
                    return_max_len=False,
                    return_num_token=False,few_shot=True,k=k,n=n,batch_label=batch_label)
                if len(all_dev_batches) < dev_count:
                    all_dev_batches.append(batch_data)
...
class FintuneProcessor(DataProcessor):
    """Processor for the MRPC data set (GLUE version)."""
        ...
        def instance_reader():
            ...
            for iter in range(iter_sum):
                ...
                for i, class_name in enumerate(target_classes):
                    ...
                    support_set = support_set + random_list[0:n]
                    query_set = query_set + random_list[n:]
                    support_set_label = support_set_label + [int(i)]*n
                    query_set_label = query_set_label + [int(i)]*q
                ...
                for i, support in enumerate(support_set):
                    ...
                    sample = InputExample(guid="", text_a=support.text_a, text_b=None, label=int(i))
                    feature = self.convert_example(0,sample, None, self.max_seq_len, self.tokenizer)
                    instance = self.generate_instance(feature)
                    task.append(instance)
                #for i in range(1):
                yield task,support_set_label
                
                task = []
                for i, query in enumerate(query_set):
                    
                    sample = InputExample(guid="", text_a=query.text_a, text_b=None, label=int(i))
                    feature = self.convert_example(0,sample, None, self.max_seq_len, self.tokenizer)
                    instance = self.generate_instance(feature)
                    task.append(instance)
                yield task,support_set_label             
