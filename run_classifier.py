...
train_g.add_arg("train_iter",             int,    30000,       "num of iters in training.")
train_g.add_arg("val_iter",             int,    1000,       "num of iters in validation.")
train_g.add_arg("test_iter",             int,    1000,       "num of iters in testing.")

train_g.add_arg("k",             int,    0,       "Number of k way.")
train_g.add_arg("n",             int,    0,       "Number of n shot.")
train_g.add_arg("q",             int,    0,       "Number of q.")
...
def evaluate(exe, test_program, test_data_loader, fetch_list, eval_phase, task_name,test_prog):
    ...
    for data in test_data_loader():
        steps = steps + 1
        #if task_name == "fintune":
        #test_program = train_compiled_program
        np_loss = None 
        np_acc = None 
        np_num_seqs = None
        if task_name == "fintune":
            if steps % 2 == 0:
                np_loss, np_acc, np_num_seqs = exe.run(program=test_prog, feed = data,
                fetch_list=fetch_list)
            else:
                np_loss, np_acc, np_num_seqs = exe.run(program=test_program, feed = data,
                fetch_list=fetch_list)
        if task_name == "fintune":
            if steps % 2 == 0:
...
def main(args):
    ...
    processors = {
        ...
        'fewshot':reader.FewshotProcessor,
        'fintune':reader.FintuneProcessor
    }
    ...
    train_program_test = fluid.Program()
        ...
        with fluid.program_guard(train_program, startup_prog):
            with fluid.unique_name.guard():
                train_data_loader, loss, probs, accuracy, num_seqs = create_model(
                    ...
                    num_labels=num_labels, k=args.k,n=args.n,q=args.q,task_name=task_name)
    ...
    if args.do_train:
        ...
        for data in train_data_loader():
            ...
            if task_name == "fintune":
                if steps % 2 == 0:
                    outputs = exe.run(test_prog , feed=data, fetch_list=fetch_list)
                else:
                    outputs = exe.run(train_compiled_program, feed=data, fetch_list=fetch_list) 
            else:
            ...
            if steps % args.validation_steps == 0:
                ...
                if args.do_val:
                    evaluate(exe, dev_prog, dev_data_loader,
                        ...
                        "dev",task_name=task_name,test_prog = test_prog)
                # evaluate test set
                if args.do_test:
                    evaluate(exe, test_prog, test_data_loader,
                        [loss.name, accuracy.name, num_seqs.name],
                        "test",task_name=task_name,test_prog = test_prog)           
    ...
    if args.do_val:
        ...
        evaluate(exe, dev_prog, dev_data_loader,
                 [loss.name, accuracy.name, num_seqs.name], "dev", task_name=task_name,test_prog = test_prog)

    # final eval on test set
    if args.do_test:
        print("Final test result:")
        evaluate(exe, test_prog, test_data_loader,
                 [loss.name, accuracy.name, num_seqs.name], "test", task_name=task_name,test_prog = test_prog)
